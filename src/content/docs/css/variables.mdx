...
Which CSS variables should you use for each project? What are they?

## What are CSS variables?

CSS variables, or also called custom properties, are variables that we can define to represent specific values. These values can then be reused throughout a project.

For example, `--primary-color: blue;` creates the `--primary-color` variable that we can use to style different elements like so: `p { color: var(--primary-color) }`.
...
Select colors as described on [this page](/visual-design/colors). Define them in one place to make them easily reuseable, this way we do not have slight variations in color when do not want that, and it makes it easier for the overall design to feel coherent. And if needed we can change the colors at one place, this is especially useful when we want to have different color themes such as light and dark mode.
